...
implements RandomAccessData {
    ...
    public byte[] read(long offset, long length) throws IOException {
        ...
        this.read(bytes2, offset, 0, bytes2.length);
        return bytes2;
    }
    ...
    private int read(byte[] bytes2, long position, int offset, int length) throws IOException {
        if (position > this.length) {
            return -1;
        }
        return this.fileAccess.read(bytes2, this.offset + position, offset, length);
    }
    ...
    private static final class FileAccess {
        ...
        private int read(byte[] bytes2, long position, int offset, int length) throws IOException {
            ...
            synchronized (object) {
                ...
                return this.randomAccessFile.read(bytes2, offset, length);
            }
        }
