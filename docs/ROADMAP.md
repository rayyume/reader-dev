...
> 状态：**Rust 重构已成型（v5.0.8）**——核心功能全部落地；本文档记录已完成项与剩余待办。
> 更新日期：2026-08-08。版本号以 `Cargo.toml` 为准（当前 `5.0.8`）。
...
- [x] 管理员历史命名空间归位 default：旧版 username 命名空间的书架/书源/规则启动时自动并入，幂等且 default 冲突保留（v5.0.8）
